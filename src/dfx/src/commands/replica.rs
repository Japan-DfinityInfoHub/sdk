...
fn get_config(
    ...
    let port = get_port(config, opts.port.as_ref())?;
...
pub fn exec(env: &dyn Environment, opts: ReplicaOpts) -> DfxResult {
    ...
    let replica_configuration_path = local_server_descriptor.replica_configuration_dir();
    fs::create_dir_all(&replica_configuration_path).with_context(|| {
        format!(
            "Failed to create replica config directory {}.",
            replica_configuration_path.to_string_lossy()
        )
    })?;
